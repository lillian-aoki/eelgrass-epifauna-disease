...
Same as final SEM used in paper but without Bodega values - this is to test if there is an effect of bias with the smaller animals in the BB samples but not elsewhere.
Gonna be messed up due to temperature anyway...goal is to check that overall results hold - i.e. epifauna and snails have positive relationship with prevalence and lesion area. And they do! Good. 
...
Idoteids don't predict lesion area without BB sites included. However, idoteids are not typically small (in the 0.5-1 mm category) so seems unlikely that they are strongly biased by wonky BB data. And, snail relationship holds.   
